...
const NFT_ID = 5;
const IMAGES_DIR = process.env.ASSETS_DIR || path.join(__dirname, '..', 'assets', 'epic');

// Dados dos jogadores do Flamengo
const playersData = {
  'MENGO - 1 - ROSSI.png': {
    name: 'Agustín Rossi',
    position: 'Goleiro',
...
async function uploadImageToS3(filePath, fileName) {
  try {
    ...
    const key = `flamengo-assets/${Date.now()}-${fileName}`;
...
async function processImages() {
  try {
    console.log('🚀 Iniciando processamento das imagens do Flamengo...\n');
    ...
    for (const file of files) {
      ...
      const assetData = {
        nft_id: NFT_ID,
        title: `${playerData.name} - Camisa ${playerData.number}`,
        description: `${playerData.description} Jogador do Clube de Regatas do Flamengo, o Mais Querido do Brasil. ${playerData.position} que veste a camisa ${playerData.number}.`,
        image_url: imageUrl,
        asset_data: {
        ...
        metadata: {
          name: `${playerData.name} - Flamengo #${playerData.number}`,
      ...
      console.log(`✅ ${playerData.name} processado com sucesso!\n`);
    }
    ...
    fs.writeFileSync(
      path.join(__dirname, 'flamengo-assets-summary.json'),
      JSON.stringify(summary, null, 2)
    );

    console.log('🎉 PROCESSAMENTO CONCLUÍDO!');
    console.log(`📊 Total de assets processados: ${processedAssets.length}`);
    console.log(`💾 Resumo salvo em: scripts/flamengo-assets-summary.json`);
    console.log(`🏆 Todos os assets foram relacionados ao NFT ID: ${NFT_ID}`);

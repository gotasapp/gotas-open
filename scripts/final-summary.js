...
async function generateFinalSummary() {
  try {
    console.log('📊 Gerando resumo final dos assets do Flamengo...\n');

    // Buscar todos os assets do NFT ID 5
    const query = `
      SELECT 
        id,
        title,
    ...
    console.log('📋 RESUMO POR POSIÇÃO:\n');
    ...
    const finalSummary = {
      ...
      s3_folder: "flamengo-assets"
    };

    // Salvar resumo final
    const summaryPath = path.join(__dirname, 'flamengo-final-summary.json');
    ...
    console.log(`🏆 Total de ${assets.length} assets do Flamengo criados para o NFT ID 5`);

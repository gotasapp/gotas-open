...
export const ImageSwiper: React.FC<ImageSwiperProps> = ({
  ...
  return (
    <section
      className={`relative grid place-content-center select-none w-full h-full ${className}`}
      ref={cardStackRef}
      style={{
        touchAction: 'none',
        ...
        '--card-swap-duration': '0.5s',
      } as React.CSSProperties}
    >
      {cardOrder.map((originalIndex, displayIndex) => {
        const isFrontCard = displayIndex === 0;

        return (
          <article
            key={`${imageList[originalIndex]}-${originalIndex}`}
            className="image-card absolute cursor-grab active:cursor-grabbing
                       place-self-center rounded-[10px] border-[10px] border-black/20
                       shadow-md overflow-hidden will-change-transform w-full h-full"
            style={{
          ...
          >
            ...
            <div
              className={`absolute inset-0 bg-black/90 backdrop-blur-sm transition-opacity duration-500
                         ${isFrontCard ? 'opacity-0' : 'opacity-100'}`}
